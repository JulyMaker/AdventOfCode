...
Assembunny::Assembunny(std::vector<std::string>& instrucc)
{
  for (const auto& line : instrucc) 
  {
...
Assembunny& Assembunny::optMul()
{
  for (unsigned int p= 5; p < ins.size(); ++p) {
...
Assembunny& Assembunny::optAdd()
{
  for (unsigned int p= 2; p < ins.size(); ++p) {
...
Assembunny& Assembunny::optRemoveInfiniteLoops()
{
...
int& Assembunny::ref(char c)
{
  return regs[c];
}

int Assembunny::get(char c)
{
  return ref(c);
}

Assembunny& Assembunny::set(char c, int v)
{
  ref(c) = v;
  return *this;
}

Assembunny& Assembunny::setOutputBufferLimit(int outLimit)
{
  this->outLimit = outLimit;
  return *this;
}

std::string Assembunny::getOuputBuffer()
{
  return out;
}

Assembunny& Assembunny::clearOutputBuffer()
{
  out.clear();
  return *this;
}