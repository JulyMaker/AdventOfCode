...
bool memoized(const Game& game)
{
  static std::unordered_set <Game, GameHash, GameEq> states;
  return !states.insert(game).second;
}

void round(bool part2, Game game, int spell, short & best)
{
  if (game.playerTurn(spell, best) || game.effects(best) || game.hardMode(part2) || game.effects(best) || game.bossTurn() || memoized(game))
    return;

  for (int i = 0; i < 5; ++i)
    round(part2, game, i, best);
}
