#ifndef UTIL_A_STARN_H_
#define UTIL_A_STARN_H_
...
std::vector<Node> BacktrackPath( Node end, const std::unordered_map<Node, Node, stateNHash>& came_from) 
{
...
auto AStarN(Node start, Node goal, AdjacentFn get_adjacent, CostFn edge_cost)-> std::optional<BestPathN<Node, decltype(edge_cost(start, goal))>> 
{
  ...
  std::unordered_map<Node, Node, stateNHash> came_from;
  std::unordered_map<Node, Cost, stateNHash> cost_to_node{ {start, Cost(0)} };
  ...
  while (!node_queue.empty()) 
  {
    ...
    {
      return BestPathN<Node, Cost>{.path = BacktrackPath(goal, came_from),
        .cost = current.cost};
    }
